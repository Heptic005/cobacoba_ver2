...
const List<bool> _kTabularNumeric = [
  false,
  false,
  false,
  false,
...
String _fmtCurrency(num v) => NumberFormat.currency(
  locale: 'id_ID',
  symbol: 'Rp ',
  decimalDigits: 0,
).format(v);
...
class ReportExportService {
  ...
  Future<Uint8List> buildReportPdf(
    ...
    return await compute<_PdfPayload, Uint8List>(
      _buildReportPdfIsolate,
      payload,
    );
  }
  ...
  Future<void> previewReportPdf(
    ...
    if (rows.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Tidak ada data untuk preview')),
      );
      return;
    }
  ...
  Future<String> buildReportXlsx(
    ...
    final path = await _saveBytesWithFallback(
      Uint8List.fromList(fileBytes),
      _suggestFile(meta, 'xlsx'),
    );
    return path ?? '';
  }

  Future<String> buildReportCsv(
    List<ReportRowData> rows, {
    required Map<String, Object?> meta,
  }) async {
              ...
              child: Column(
                children: [
                  ...
                  Align(
                    ...
                    child: Text(
                      'Menampilkan ${shown.length} dari $total baris',
                  ...
                  LayoutBuilder(
                    builder: (c, constraints) {
                      final maxH =
                          ...
                          0.62; // occupy up to ~62% of screen height
                    ...
                    children: [
                        ...
                        onPressed: () async {
                          ...
                          try {
                            if (title.toLowerCase().contains('excel')) {
                              final path = await buildReportXlsx(
                                rows,
                                meta: meta,
                              );
                              if (path.isNotEmpty)
                              ...
                              final path = await buildReportCsv(
                                rows,
                                meta: meta,
                              );
                              if (path.isNotEmpty)
                                ScaffoldMessenger.of(context).showSnackBar(
                                  SnackBar(
                                    content: Text('CSV disimpan: $path'),
                                  ),
                                );
                            }
                          } catch (e) {
                      ...
                      OutlinedButton.icon(
                        onPressed: () async {
                          ...
                          try {
                            ...
                            await Printing.sharePdf(
                              bytes: bytes,
                              filename: _suggestFile(meta, 'pdf'),
                            );
                          } catch (e) {
                            ScaffoldMessenger.of(context).showSnackBar(
                        ...
                        style: OutlinedButton.styleFrom(
                          ...
                          side: BorderSide(color: kInputBg),
                        ),
                      ),
                    ],
                  ),
  ...
  Future<String?> _saveBytesWithFallback(
    ...
    try {
      ...
      final outDir = Directory(
        '${dir.path}${Platform.pathSeparator}DakaraExports',
      );
  ...
  String _cacheKey(Map<String, Object?> meta, List<ReportRowData> rows) {
    ...
    return 'report_${encoder.encode(seed)}.bin'.replaceAll(
  ...
  String _suggestFile(Map<String, Object?> meta, String ext) {
    ...
    final start =
        startVal is DateTime
            ? DateFormat('yyyyMMdd').format(startVal)
            : (startVal?.toString().split(' ').first ?? 'all');
    final end =
        endVal is DateTime
            ? DateFormat('yyyyMMdd').format(endVal)
            : (endVal?.toString().split(' ').first ?? 'all');
  ...
  pw.Widget header() {
    return pw.Column(
      ...
      children: [
        ...
        pw.Text(
          'Periode: ${metaStr('startDate')} s.d. ${metaStr('endDate')}',
          style: const pw.TextStyle(fontSize: 11),
        ),
        if ((meta['search'] ?? '').toString().isNotEmpty)
          pw.Text(
            'Pencarian: ${meta['search']}',
            style: const pw.TextStyle(fontSize: 11),
          ),
        pw.Text(
          'Dicetak: ${dateFmt.format(DateTime.now())}',
    ...
    pw.MultiPage(
                ...
                pw.Expanded(
                  child: pw.Column(
                    ...
                    children: [
                      ...
                      pw.Text(
                        'Jl. Contoh No.123, Kota Contoh, Telp: (021) 123-4567',
                        style: pw.TextStyle(
                          fontSize: 9,
                          color: PdfColors.grey700,
                        ),
      ...
      build: (context) {
          ...
          rowsTable.add(
            pw.TableRow(
              children: [
                ...
                cellContainer(
                  numFmt.format(r['bruto'] ?? 0),
                  bg,
                  align: pw.TextAlign.right,
                ),
                cellContainer(
                  numFmt.format(r['tare'] ?? 0),
                  bg,
                  align: pw.TextAlign.right,
                ),
                cellContainer(
                  numFmt.format(r['netto'] ?? 0),
                  bg,
                  align: pw.TextAlign.right,
                ),
                cellContainer(
                  currencyFmt.format(r['totalPrice'] ?? 0),
                  bg,
                  align: pw.TextAlign.right,
                ),
              ],
        ...
        return [
          ...
          pw.Container(
            ...
            child: pw.Row(
              ...
              children: [
                ...
                pw.SizedBox(width: 12),
                pw.Text(
                  '${numFmt.format(totalBruto)} kg',
                  style: pw.TextStyle(fontSize: 10),
                ),
                pw.SizedBox(width: 12),
                pw.Text(
                  '${numFmt.format(totalTare)} kg',
                  style: pw.TextStyle(fontSize: 10),
                ),
                pw.SizedBox(width: 12),
                pw.Text(
                  '${numFmt.format(totalNetto)} kg',
                  style: pw.TextStyle(fontSize: 10),
                ),
                pw.SizedBox(width: 12),
                pw.Text(
          ...
          pw.Row(
            mainAxisAlignment: pw.MainAxisAlignment.spaceBetween,
            children: [_signBox('Disetujui'), _signBox('Dibuat')],
          ),
        ];
      },
    ),
