			...
			children: [
				Row(
					children: [
						...
						IconBtn(icon: Icons.filter_list, tooltip: 'Filter', onTap: onShowFilter),
						const SizedBox(width: 8),
				...
				Row(
					children: [
						DateButton(date: controller.startDate, hint: 'Start', onTap: onPickStartDate),
						const SizedBox(width: 8),
						DateButton(date: controller.endDate, hint: 'End', onTap: onPickEndDate),
						const SizedBox(width: 12),
						Expanded(
							child: Row(
								...
								children: [
									ActionButton(
										icon: controller.sortDesc ? Icons.arrow_downward : Icons.arrow_upward,
					...
					child: Column(
						...
						children: [
							...
							FilterDropdown<int?>(
								...
								items: [null, ...controller.suppliers.value.map((s) => s.supplierId)],
								labelFn: (id) => id == null ? 'Semua Supplier' : getSupplierName(id),
								onChanged: (v) {
							...
							FilterDropdown<int?>(
								...
								items: [null, ...controller.customers.value.map((c) => c.customerId)],
								labelFn: (id) => id == null ? 'Semua Customer' : getCustomerName(id!),
								onChanged: (v) {
							...
							Column(
								crossAxisAlignment: CrossAxisAlignment.start,
								children: [
									const Text('Plat Kendaraan', style: TextStyle(color: kTextGrey, fontSize: 12)),
									...
									Container(
										...
										child: TextField(
											controller: TextEditingController(text: controller.plate ?? ''),
											style: const TextStyle(color: kTextWhite),
											decoration: const InputDecoration(border: InputBorder.none, hintText: 'Masukkan plat (mis. B1234CD)'),
							...
							SizedBox(
								...
								child: OutlinedButton(
									...
									child: const Text('Reset Semua Filter', style: TextStyle(color: kTextGrey)),
								),
							),
						],
					),
